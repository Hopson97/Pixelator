...
{
	if (size % pixelSize != 0) {
		std::cout << "ERROR: Image " << desc
			  << " is not divisible by pixel size.\n";
...
int main(int argc, char **argv)
{
	...
	if (!((std::string)argv[2]).compare("auto")) {
		pixelSize = GCD(image.getSize().x, image.getSize().y);
		printf("Greatest common divisor was found to be: %d\n",
		       pixelSize);
	} else
		pixelSize = std::stoi(argv[2]); //TODO Error check
	...
	std::thread visualiseThread([&]() { visualise(image, newImage); });
	...
	for (auto &pos : pxlPositions) {
		...
		sf::Color avgColour{ uint8_t(totalRed / total),
